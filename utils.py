...
def update_node_embeddings(ent_adj, ent_emb, node_size, node_hidden):
    """
    更新节点的嵌入表示为其相邻节点的嵌入表示的平均值。

    参数:
        ent_adj (torch.Tensor): 边的张量，shape=(node_size, 2)，表示节点间的连接。
        ent_emb (torch.Tensor): 节点的嵌入表示，shape=(r_val,r_index的长度即不同的(h, t)对数, hidden_size)。

    返回:
        torch.Tensor: 更新后的节点嵌入表示。
    """
    # 初始化累加器，用于存放每个节点嵌入的累加和
    accumulator = torch.zeros_like(torch.empty(node_size, node_hidden)).to(ent_emb.device)
    # 初始化计数器，用于记录每个节点相邻节点的数量
    count = torch.zeros(node_size).to(ent_emb.device)

    ent_adj = torch.tensor(ent_adj).to(ent_emb.device)
    # 遍历每条边，累加相邻节点的嵌入表示
    
    for start_node, end_node in ent_adj:
        ent_emb_vector = F.embedding(end_node, ent_emb) # 之前是
        accumulator[start_node] += ent_emb_vector
        count[start_node] += 1

    # ent_adj有自环,所以没有count为0
    # 避免除以0的情况，将计数器中的0替换为1（对于孤立节点，它们的嵌入将保持不变）
    count[count == 0] = 1
    # 计算平均值
    #print(accumulator.shape, count.shape) # torch.Size([341396, 128]) torch.Size([39594])
    count = count.unsqueeze(1)
    updated_emb = accumulator / count
